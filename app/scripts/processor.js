...
importScripts('/app/scripts/dexie.js');
...
var calcMetrics = function (startTime, endTime, activityID, userUnit) {
	...
	for (i = start; i < end; i++) {
		...
		if (i !== 0) {
			timeSectionMSeconds = activity[i].date - activity[i-1].date;
		}
	...
	metrics.pace.maxPace.value = unit.speedValue(metrics.pace.maxPace.value, userUnit);
